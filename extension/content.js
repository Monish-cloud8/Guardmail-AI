...
scanButton.onclick = function() {
    ...
    fetch('http://127.0.0.1:3000/api/analyze-ext', {
        method: 'POST',
    ...
    .then(data => {
        ...
        if (insertionTarget) {
            ...
            ribbon.innerHTML = `
                ...
                <a href="http://127.0.0.1:3000/?emailId=${data.id}" target="_blank" style="background:#4f46e5; color:#ffffff; font-weight:800; font-size:11px; padding:8px 14px; border-radius:10px; text-decoration:none; text-transform:uppercase; letter-spacing:0.5px; white-space:nowrap;">Inspect Sandbox Analysis</a>
            `;
    ...
    .catch(err => {
        console.error(err);
        alert("Connection Failure: Ensure your Python Flask app engine is currently processing local port traffic at http://127.0.0.1:3000");
        scanButton.innerHTML = `<i class="fas fa-shield-halved"></i> Audit with GuardMail AI`;
        scanButton.style.background = 'linear-gradient(135deg, #4f46e5, #6366f1)';
    });
};